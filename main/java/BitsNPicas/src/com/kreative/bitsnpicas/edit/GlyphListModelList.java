...
public class GlyphListModelList extends JTree {
	...
	private static class GlyphListModelRootNode extends GlyphListModelTreeNode {
		public GlyphListModelRootNode(Font<?> font) {
			...
			GlyphListModelTreeNode allGlyphs = new GlyphListModelTreeNode(this, new GlyphListFontModel(font, true, true, "全部字形"));
			allGlyphs.children.add(new GlyphListModelTreeNode(this, new GlyphListFontModel(font, true, false, "全部编码字形")));
			allGlyphs.children.add(new GlyphListModelTreeNode(this, new GlyphListFontModel(font, false, true, "全部命名字形")));
	...
	private static class GlyphListModelEncodingNode extends GlyphListModelTreeNode {
		public GlyphListModelEncodingNode(GlyphListModelTreeNode parent, Encoding encoding) {
			...
			for (int i = 0; i < 256; i++) {
				...
				if (s != null) {
					String n = "子表 " + Integer.toHexString(0xFF00 | i).substring(2).toUpperCase();
